...
impl<K: Hash<S> + Default + Clone, V: Default + Clone, S, H: Hasher<S>>
	...
	fn find_closer_bucket(&mut self, free_distance:&mut uint, index_addr:uint,
		...
		while 0 < free_dist {
		    let mut move_bucket_hop_info = self.raw_table.get_bucket(
    ...
    pub fn insert(&mut self, key:K, data:V)-> bool{
		...
		let mut val = 1;
		//probes through the hash table, and checks if there is an empty bucket
		//within ADD_RANGE.
		...
		if free_distance < ADD_RANGE {
			...
			while val != 0 {
				...
				//If it is, insert the bucket, if not, we try and displace it
				//closer.
				...
				self.find_closer_bucket(&mut free_distance, index_addr,
																&mut val, mask);
			}
		}
	    self.resize();
