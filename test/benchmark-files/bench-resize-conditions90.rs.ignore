...
fn main(){
    ...
    for ar in add_ranges.iter(){
        let mut res = 0;
        for i in range(0,100){
            if resize_test(29491,32768,*ar){
                res += 1;
            }
        }
        size_32768.push(res);
    }
    let mut file = File::create(&Path::new("output-files/bench-resize-conditions90"));
