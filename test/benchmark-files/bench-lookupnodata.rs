...
fn insert_lookup_remove_robin(load:uint)->u64{
    let mut m:HashMap<uint,uint> = HashMap::with_capacity(HASHMAP_CAPACITY);
...
fn insert_lookup_remove_hopscotch(load:uint)->u64{
    let mut m:hopscotch::HashMap<uint,uint> = hopscotch::HashMap::with_capacity(HASHMAP_CAPACITY);
	let start = precise_time_ns();
    for i in range(0,load){
        m.lookup(i);
    }
...
fn main(){
    ...
    for i in data_point.iter(){
        ...
        let time_hopscotch = (sum_nano as f64)/(*i as f64) - Hash_OH;
        ...
        let time_robin = (sum_nano2 as f64) / (*i as f64) - Hash_OH;
        result_robin.push(time_robin);
    }

    let mut file = File::create(&Path::new("output-files/bench-lookupsnodata"));
