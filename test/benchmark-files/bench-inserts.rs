...
fn insert_lookup_remove_hopscotch(load:uint)->u64{
	let h = hashers::HasherZeroAndUp;
    let mut m = hopscotch::HashMap::with_capacity_and_hasher(HASHMAP_CAPACITY, h);
    let start = precise_time_ns();
    for i in range(0,load){
        m.insert(i,i+1);
    }
...
fn main(){
    ...
    let mut file = File::create(&Path::new("output-files/bench-inserts"));
